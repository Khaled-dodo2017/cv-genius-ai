...
// listen to inputs, textareas and selects so the preview updates for all common form controls  
...
return Array.from(  
  ...
  .map((entry) => {  
    ...
    Object.entries(fields).forEach(([key, selector]) => {  
      ...
      // guard access to .value and trim safely  
      const raw = field?.value ?? '';  
      result[key] = typeof raw === 'string' ? raw.trim() : '';  
    });  
...
if (out.name) out.name.textContent = name || 'Your Name';  
if (out.title) out.title.textContent = jobTitle || 'Job Title';  
...
if (skills.length) {  
  ...
  if (sections.skills) sections.skills.hidden = false;  
} else {  
  if (out.skills) out.skills.innerHTML = '';  
  if (sections.skills) sections.skills.hidden = true;  
}  
...
if (languages.length) {  
  ...
  if (sections.languages) sections.languages.hidden = false;  
} else {  
  if (out.languages) out.languages.innerHTML = '';  
  if (sections.languages) sections.languages.hidden = true;  
}
...
if (!items.length) {
if (sections.experience) sections.experience.hidden = true;
if (out.experience) out.experience.innerHTML = '';
return;
}

if (sections.experience) sections.experience.hidden = false;  

if (!out.experience) return;  
...
if (!items.length) {
if (sections.education) sections.education.hidden = true;
if (out.education) out.education.innerHTML = '';
return;
}

if (sections.education) sections.education.hidden = false;  

if (!out.education) return;  
...
if (experienceList) experienceList.innerHTML = '';  
  ...
  if (experienceList && experienceTemplate && !experienceList.querySelector('[data-entry]')) {  
    addEntry(  
      experienceList,  
      experienceTemplate  
    );  
  ...
  if (educationList && educationTemplate && !educationList.querySelector('[data-entry]')) {  
    addEntry(  
      educationList,  
      educationTemplate  
    );  
...
const formData = new FormData(form);  
  ...
  // Collect repeated fields into arrays instead of overwriting  
  for (const [key, value] of formData.entries()) {  
    if (Object.prototype.hasOwnProperty.call(cvData, key)) {  
      if (!Array.isArray(cvData[key])) cvData[key] = [cvData[key]];  
      cvData[key].push(value);  
    } else {  
      cvData[key] = value;  
    }  
  }  
...
if (!Array.isArray(savedCVs) || !savedCVs.length) {  
